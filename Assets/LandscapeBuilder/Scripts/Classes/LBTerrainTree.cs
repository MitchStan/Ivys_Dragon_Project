...
namespace LandscapeBuilder
{
    ...
    public class LBTerrainTree
    {
        ...
        /// <summary>
        /// Does tree match the search criteria.
        ...
        {
            bool isMatch = false;
            string treeNameText = string.Empty;

            // If the search string is empty, always return true.
            if (string.IsNullOrEmpty(searchFilter)) { isMatch = true; }
            else
            {
                if (prefab != null) { treeNameText = prefab.name; }
                else if (!string.IsNullOrEmpty(prefabName)) { treeNameText = prefabName + " N/A"; }
                else { treeNameText = "No tree prefab"; }

                isMatch = (!string.IsNullOrEmpty(treeNameText) && treeNameText.ToLower().Contains(searchFilter.ToLower()));
            }
